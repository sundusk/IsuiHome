...
export const seedMissionControl = mutation({
  args: {},
  handler: async (ctx) => {
    const [tasks, cronJobs, memories, agents] = await Promise.all([
    ...
    await Promise.all([
      ...
      ...cronJobs.map((doc) => ctx.db.delete(doc._id)),
      ...memories.map((doc) => ctx.db.delete(doc._id)),
      ...agents.map((doc) => ctx.db.delete(doc._id))
    ]);
